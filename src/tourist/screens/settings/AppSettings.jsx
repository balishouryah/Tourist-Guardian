...
export default function AppSettings() {
  ...
  const settings = [
    ...
    { key: 'language', label: 'Language & Region', desc: 'English (US), Timezone.', isRoute: true, path: '/tourist/settings/language' },
    { key: 'family-tracking', label: 'Family Tracking', desc: 'Share your live location with trusted family members.', isRoute: true, path: '/tourist/settings/family-tracking' }
  ];
    ...
    <div style={{ flex: 1, padding: 'var(--space-lg)', display: 'flex', flexDirection: 'column' }} className="animate-fade-in">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
        {settings.map(setting => (
          <div 
            key={setting.key} 
            onClick={() => setting.isRoute ? navigate(setting.path) : toggle(setting.key)}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-md)', background: 'var(--surface-container-lowest)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-card)', cursor: setting.isRoute ? 'pointer' : 'default' }}>
